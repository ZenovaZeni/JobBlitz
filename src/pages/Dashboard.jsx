  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: '#f7f9fb' }}>
      ...
      <main className="flex-1 px-4 md:px-8 lg:px-12 py-6 md:py-10 overflow-y-auto pb-24 md:pb-10">
        ...
        {/* Orientation Layer (Wizard) */}
        {!hasSessions && showOnboarding && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#f7f9fb]/80 backdrop-blur-sm animate-in fade-in duration-500">
            <div className="w-full max-w-2xl transform animate-in slide-in-from-bottom-8 duration-500">
              <div className="text-center mb-8">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#e1e0ff] text-[#0e0099] text-[10px] font-black uppercase tracking-widest mb-4">
                  <span className="material-symbols-outlined text-[14px]">rocket_launch</span>
                  First Time Here?
                </div>
                <h2 className="text-3xl font-extrabold text-[#031631] tracking-tight mb-2">Let's get you hired.</h2>
                <p className="text-[#8293b4] font-medium">Take 2 minutes to set up your first tailoring session.</p>
              </div>
        ...
        {/* Dashboard Content */}
        <div className={( !hasSessions && showOnboarding ) ? 'blur-sm pointer-events-none opacity-50 grayscale transition-all' : 'transition-all duration-500'}>
          {!hasSessions && !showOnboarding ? (
            /* EMPTY STATE VERSION OF DASHBOARD */
            <div className="py-10 md:py-20 text-center max-w-2xl mx-auto">
              <div className="w-20 h-20 bg-white rounded-3xl shadow-xl flex items-center justify-center mx-auto mb-8 rotate-3 border">
                <span className="material-symbols-outlined text-[40px] text-[#0e0099] icon-filled">dashboard_customize</span>
              </div>
              <h2 className="text-3xl font-extrabold text-[#031631] tracking-tight mb-4">Your Dashboard is Ready.</h2>
              <p className="text-lg text-[#8293b4] font-medium mb-10">
                You haven't started any application sessions yet. Start your first one to see tailored analytics and resume versions here.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button onClick={() => navigate('/app/profile')} 
                  className="flex flex-col items-center gap-4 p-8 bg-white rounded-3xl border-2 border-transparent hover:border-[#0e0099]/10 hover:shadow-2xl transition-all group">
                  <div className="w-12 h-12 rounded-2xl bg-[#f2f4f6] flex items-center justify-center group-hover:bg-[#e1e0ff] transition-colors">
                    <span className="material-symbols-outlined text-[#0e0099]">account_circle</span>
                  </div>
                  <div className="text-center">
                    <p className="font-bold text-[#031631]">Complete Profile</p>
                    <p className="text-xs font-semibold text-[#8293b4]">Set up your master resume</p>
                  </div>
                </button>
                <button onClick={() => navigate('/app/tailor')}
                  className="flex flex-col items-center gap-4 p-8 bg-[#031631] rounded-3xl hover:shadow-2xl hover:shadow-blue-900/20 transition-all group">
                  ...
                  <div className="text-center text-white">
                    <p className="font-bold">Start First Session</p>
                    <p className="text-xs font-semibold opacity-60">Tailor for a specific job</p>
                  </div>
                </button>
              </div>
            </div>
